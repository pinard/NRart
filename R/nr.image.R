"nr.image" <-
function(expr, centre=c(0, 0), zoom=1,
         ...
         main=deparse(substitute(expr)), ...)
{
    ...
    {
        display(Arg(eval(name)), main=main, bty='n', xaxt='n', yaxt='n', ...)
        mtext(paste('step', '=', step), side=1, line=2, font=2, adj=.9)
        axis(side=1,
             at=c(0, .5, 1),
             labels=c(format(xrange[1], digits=3),
                      format(centre[1], digits=3),
                      format(xrange[2], digits=3)))
        axis(side=2,
             at=c(0, .5, 1),
             labels=c(paste(format(yrange[1], digits=3), 'i', sep=''),
                      paste(format(centre[2], digits=3), 'i', sep=''),
                      paste(format(yrange[2], digits=3), 'i', sep='')))
    }
    for (step in 1:steps) {
